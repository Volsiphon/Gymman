// BACKUP — original ExecutionPlanScreen.tsx
// See RESTORE.md for instructions on restoring this file.
...
export function ExecutionPlanScreen({ navigation }: Props) {
  ...
  if (!profile) {
    return (
      <View style={{ flex: 1, backgroundColor: colors.bg.app, justifyContent: 'center', alignItems: 'center' }}>
  ...
  const proteinGrams = profile.proteinG ?? 0;
  const tdee = profile.tdee ?? 0;
  const lbmKg = profile.lbmKg ?? 0;
  ...
  function renderBodyComp() {
    ...
    const deficitLabel =
      goalType === 'fat-loss'
        ? `Eat below ${tdee} kcal and your body burns stored fat to cover the gap. A 500 kcal daily deficit = roughly 0.5 kg of fat gone per week.`
        : goalType === 'muscle-gain'
        ? `Eat above ${tdee} kcal and your body has surplus energy to build new tissue. Keep it small — too big and the extra goes to fat, not muscle.`
        : `Eat at exactly ${tdee} kcal. Your body taps into fat stores to fund muscle-building. It's the slowest path but the most efficient one.`;
      ...
      <>
        ...
        <Animated.View style={[styles.card, a(1)]}>
          ...
          <Text style={styles.cardBody}>
            Your body runs on calories. Every day you burn roughly {tdee} kcal just existing and moving around. That number is your maintenance — the tipping point between losing weight and gaining it.
          </Text>
          {renderEnergyStates()}
        ...
        <Animated.View style={[styles.card, a(2)]}>
          ...
          <View style={styles.metaRow}>
            ...
            <Text style={styles.metaText}>
              {Math.abs(calorieTarget - tdee)} kcal{' '}
              {goalType === 'fat-loss'
                ? 'below'
                : goalType === 'muscle-gain'
                ? 'above'
                : 'at'}{' '}
              your maintenance of {tdee} kcal. Expected pace: {weeklyProgressLabel}.
            </Text>
          </View>
        ...
        <Animated.View style={[styles.card, a(3)]}>
          ...
          <Text style={styles.cardBody}>
            {goalType === 'fat-loss'
              ? `A deficit tells your body to break down stored energy — but without enough protein, it breaks down muscle too. ${proteinGrams}g a day keeps your ${lbmKg} kg of lean mass intact. This number matters more than the calorie count.`
              : goalType === 'muscle-gain'
          ...
          <Text style={styles.cardMeta}>
            {(proteinGrams / lbmKg).toFixed(1)} g per kg of lean mass ·{' '}
            {lbmKg} kg lean body mass
          </Text>
        </Animated.View>
...
const styles = StyleSheet.create({
  ...
  scroll: { paddingHorizontal: spacing.screenPadding, paddingTop: spacing.xl },
  hero: { gap: spacing.sm, marginBottom: spacing.md },
  heroTitle: { ...typography.title1, color: colors.text.primary },
  heroSub: { ...typography.callout, color: colors.text.secondary, lineHeight: 23 },
  card: {
  ...
  cardTitle: { ...typography.subhead, color: colors.text.primary, fontWeight: '600', flex: 1, lineHeight: 20 },
  cardBody: { ...typography.callout, color: colors.text.secondary, lineHeight: 22 },
  cardMeta: { ...typography.footnote, color: colors.text.muted, lineHeight: 18 },
  iconWrap: { width: 32, height: 32, borderRadius: radius.md, alignItems: 'center', justifyContent: 'center', flexShrink: 0, marginTop: 1 },
  energyRow: { flexDirection: 'row', gap: spacing.sm },
  energyChip: { flex: 1, alignItems: 'center', gap: 4, paddingVertical: spacing.sm, borderRadius: radius.md, borderWidth: 1, borderColor: colors.border.default, backgroundColor: colors.bg.elevated },
  energyLabel: { ...typography.caption, color: colors.text.disabled, fontWeight: '600', textTransform: 'uppercase', letterSpacing: 0.4 },
  energyDesc: { ...typography.caption, color: colors.text.disabled, fontSize: 10 },
  targetBox: { flexDirection: 'row', alignItems: 'baseline', gap: spacing.sm, paddingVertical: spacing.xs },
  targetNumber: { fontSize: 44, fontWeight: '800', color: colors.primary, lineHeight: 50, letterSpacing: -0.5 },
  targetUnit: { ...typography.callout, color: colors.text.muted, fontWeight: '500' },
  metaRow: { flexDirection: 'row', gap: spacing.xs, alignItems: 'flex-start' },
  metaText: { ...typography.footnote, color: colors.text.muted, flex: 1, lineHeight: 18 },
  warningNote: { flexDirection: 'row', gap: spacing.sm, alignItems: 'flex-start', backgroundColor: colors.goldMuted, borderRadius: radius.md, padding: spacing.sm, borderWidth: 1, borderColor: colors.gold + '30' },
  warningNoteText: { ...typography.footnote, color: colors.text.secondary, flex: 1, lineHeight: 18 },
  infoNote: { flexDirection: 'row', gap: spacing.sm, alignItems: 'flex-start', backgroundColor: colors.infoMuted, borderRadius: radius.md, padding: spacing.sm, borderWidth: 1, borderColor: colors.info + '30' },
  primaryNote: { flexDirection: 'row', gap: spacing.sm, alignItems: 'flex-start', backgroundColor: colors.primaryMuted, borderRadius: radius.md, padding: spacing.sm, borderWidth: 1, borderColor: colors.primaryBorder },
  noteText: { ...typography.footnote, color: colors.text.secondary, flex: 1, lineHeight: 18 },
  stepList: { gap: spacing.sm },
  stepRow: { flexDirection: 'row', gap: spacing.md, alignItems: 'flex-start' },
  stepBadge: { width: 22, height: 22, borderRadius: radius.full, backgroundColor: colors.bg.elevated, borderWidth: 1, borderColor: colors.border.default, alignItems: 'center', justifyContent: 'center', flexShrink: 0, marginTop: 2 },
  stepBadgeText: { ...typography.caption, color: colors.text.muted, fontWeight: '600' },
  stepText: { ...typography.callout, color: colors.text.secondary, flex: 1, lineHeight: 22 },
  footer: { position: 'absolute', bottom: 0, left: 0, right: 0, paddingHorizontal: spacing.screenPadding, paddingTop: spacing.md, backgroundColor: colors.bg.app, borderTopWidth: StyleSheet.hairlineWidth, borderTopColor: colors.border.subtle },
  continueBtn: { height: spacing.buttonHeight, backgroundColor: colors.primary, borderRadius: radius.button, alignItems: 'center', justifyContent: 'center', flexDirection: 'row' },
  continueBtnText: { ...typography.bodyMedium, color: colors.text.inverse },
  ghostBtn: { height: 40, alignItems: 'center', justifyContent: 'center', flexDirection: 'row', marginTop: spacing.xs },
