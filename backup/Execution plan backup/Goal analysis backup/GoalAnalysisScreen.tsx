...
export function GoalAnalysisScreen({ navigation }: Props) {
  ...
  async function runAnalysis(profileOverride?: Awaited<ReturnType<typeof loadUserProfile>>) {
    setError(null);
    setLoading(true);
    const profile = profileOverride ?? await loadUserProfile();
    if (!profile) {
      setError('Could not load your profile. Please restart onboarding.');
      setLoading(false);
      return;
    }
    const s = profileToStats(profile);
    const c = computeBodyStats(s);
    try {
      const res = await analyzeGoal(s, profile.goalText, c);
      const goalType = classifyGoal(profile.goalText, c.bfPercent, s.sex);
      ...
      await saveUserProfile({
        ...
        targetWeightKg: res.journey?.targetWeightKg ?? res.alternativeGoal?.journey?.targetWeightKg,
      });
      setResult(res);
    } catch {
      setError('Something went wrong analysing your goal. Please try again.');
    } finally {
      setLoading(false);
    }
  }
  ...
  useEffect(() => {
    loadUserProfile().then(p => runAnalysis(p ?? undefined));
  }, []);
