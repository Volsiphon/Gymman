...
const hd = StyleSheet.create({
  ...
  btnRow: { flex: 1, flexDirection: 'row', gap: 6, justifyContent: 'flex-end' },
  pill: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: 4,
...
// ── Bloodwork button with crimson smoke animation ─────────────────────────────

function BloodworkButton({ onPress }: { onPress: () => void }) {
  return (
    <TouchableOpacity style={bs.btn} onPress={onPress} activeOpacity={0.75}>
      <Ionicons name="medical-outline" size={15} color={colors.text.inverse} />
      <Text style={bs.label}>BLOODWORK</Text>
    ...
    <View style={[s.root, { paddingTop: insets.top }]}>
      ...
      <View style={s.scroll}>
        ...
        <View style={s.proTipRow}>
          <Text style={s.proTipBadge}>PRO TIP</Text>
          <Text style={s.proTipText}>Tell the Coach if something feels off.</Text>
        </View>
      </View>
    </View>
  );
...
const s = StyleSheet.create({
  root:   { flex: 1, backgroundColor: colors.bg.app },
  scroll: { paddingHorizontal: spacing.screenPadding, paddingTop: spacing.xs, gap: spacing.sm + 4, paddingBottom: spacing.lg },
  proTipRow: {
    alignItems: 'center',
    marginTop: -2,
    gap: 3,
